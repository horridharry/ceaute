...
import { createTreatment } from "../actions/create-treatment";

export function CreateTreatmentUI({ userId }) {
  const [stateMessage, createTreatmentAction, pending] = useActionState(
    createTreatment,
    "",
  );
  ...
  const handleSubmit = async (formData) => {
    createTreatmentAction(formData);
  };
      ...
      <div className="mt-6 flex flex-col">
        ...
        >
          <span className="field-set">
            ...
            <label
              htmlFor="image"
              className="relative w-32 h-32 overflow-hidden cursor-pointer hover:opacity-50 duration-150"
            >
              <Image
...
const SubmitButton = ({ loading, error, pending }) => {
  return (
    ...
    >
      {!pending ? "Create treatment" : "Creating..."}
    </button>
  );
};
